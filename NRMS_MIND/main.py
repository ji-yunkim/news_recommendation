...
def main():
    ...
    for epoch in range(1, epochs + 1):
        ...
        with open(os.path.join(f'prediction-{epoch}.txt'), 'w') as f:
            ...
            # for j, (impr_idx_j, vld_his_j, vld_cand_j, vld_label_j) in tqdm(enumerate(valid_loader), desc='Evaluation', total=len(valid_loader)):
            # print(len(valid_loader))
            for j, (vld_his_j, vld_cand_j, vld_label_j) in tqdm(enumerate(valid_loader), desc='Eval', total=len(valid_loader)):
                # print(vld_his_j.shape)
                # print(vld_cand_j.shape)
                # print(vld_cand_j.shape)
                # Get model output
                vld_his_j, vld_cand_j = vld_his_j.to(device), vld_cand_j.to(device)
